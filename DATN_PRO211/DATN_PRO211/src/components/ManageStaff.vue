...
export default {
  ...
  setup() {
    ...
    const canAddStaff = computed(() =>
      authStore.canAccess([ROLES.ADMIN])
    )
    ...
    const loadStaff = async () => {
      staff.value = await authStore.getAllStaff()
    }
    ...
    const displayStaff = computed(() =>
      searchResults.value ?? staff.value
    )
    ...
    const performSearch = () => {
      ...
      searchResults.value = staff.value.filter(s =>
    ...
    const startEdit = (staffMember) => {
      ...
      formData.value = { ...staffMember, password: '' }
      showAddForm.value = false
    }
    ...
    const saveStaff = async () => {
      ...
      try {
        if (editingStaff.value) {
          ...
          await authStore.updateStaff(editingStaff.value.id, payload)
        } else {
          if (!formData.value.password) {
            formError.value = 'Vui lòng nhập mật khẩu'
            return
          }
          await authStore.addStaff(formData.value)
        }
    ...
    const deleteStaff = async (id) => {
      ...
      await authStore.deleteStaff(id)
      await loadStaff()
      searchResults.value = null
    }
