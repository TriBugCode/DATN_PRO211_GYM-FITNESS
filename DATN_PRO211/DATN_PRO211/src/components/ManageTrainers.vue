...
export default {
  ...
  setup() {
    ...
    /* ================= PERMISSION ================= */
    const canAddTrainer = computed(() =>
      authStore.canAccess([ROLES.ADMIN, ROLES.STAFF])
    )

    const canEditTrainer = computed(() =>
      authStore.canAccess([ROLES.ADMIN, ROLES.STAFF])
    )

    /* ================= LOAD DATA ================= */
    const loadTrainers = async () => {
      trainers.value = await authStore.getAllTrainers()
    }
    ...
    const displayTrainers = computed(() =>
      searchResults.value ?? trainers.value
    )
    ...
    const performSearch = () => {
      ...
      searchResults.value = trainers.value.filter(t =>
    ...
    const startEdit = (trainer) => {
      ...
      formData.value = { ...trainer, password: '' }
      showAddForm.value = false
    }

    /* ================= SAVE ================= */
    const saveTrainer = async () => {
      const { name, email, phone, specialty } = formData.value
      ...
      try {
        if (editingTrainer.value) {
          ...
          await authStore.updateTrainer(editingTrainer.value.id, payload)
        } else {
          if (!formData.value.password) {
            formError.value = 'Vui lòng nhập mật khẩu'
            return
          }
          await authStore.addTrainer(formData.value)
        }
    ...
    const deleteTrainer = async (id) => {
      ...
      await authStore.deleteTrainer(id)
      await loadTrainers()
      searchResults.value = null
    }

    watch(searchQuery, v => {
      if (!v.trim()) searchResults.value = null
    })
